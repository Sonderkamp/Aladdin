            ...
            <form action="/ProfileCheck/action=block?user={$cuser.Email}" method="post"><p>
                ...
                <p>
                    ...
                    {if ($blockstatus.IsBlocked eq 0)}
                        <input type="submit" formaction="/ProfileCheck/action=block?user={$cuser.Email}"
                               value="Blokkeer"
                               class="btn btn-default">
                    {elseif ($blockstatus.IsBlocked eq 1)}
                        <input type="submit" formaction="/ProfileCheck/action=unblock?user={$cuser.Email}"
                               value="Deblokkeer"
                               class="btn btn-default">
                    {/if}
                    <br>
                    ...
                    <td>
                        <form method="post">
                            <button type="submit" formaction="/Wishes/wish_id={$wish.wishid}"
                                    value="{$smarty.server.HTTP_HOST}{$smarty.server.REQUEST_URI}" name="page"><span
